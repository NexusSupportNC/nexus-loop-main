...
const OrganizationManagement = ({ addNotification }) => {
  ...
  const fetchOrganizations = async () => {
    try {
      ...
      if (data.success) {
        ...
        addNotification('Failed to fetch organizations', 'error');
      }
    } catch (error) {
      console.error('Error fetching organizations:', error);
      addNotification('Error fetching organizations', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteOrganization = async (orgId, orgName) => {
    if (!window.confirm(`Are you sure you want to delete "${orgName}"? This will remove all user assignments.`)) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        addNotification(data.message || 'Failed to delete organization', 'error');
      }
    } catch (error) {
  ...
  const handleUpdateOrganization = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        addNotification(data.message || 'Failed to update organization', 'error');
      }
    } catch (error) {
      console.error('Error updating organization:', error);
      addNotification('Error updating organization', 'error');
    }
  };

  const handleManageUsers = async (org) => {
    setSelectedOrg(org);
    try {
      const token = localStorage.getItem('token');
      const [usersResponse, availableResponse] = await Promise.all([
        fetch(`/api/organizations/${org.id}/users`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
        fetch(`/api/organizations/${org.id}/available-users`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      ]);
      ...
      if (usersData.success && availableData.success) {
  ...
  const handleAddUser = async (userId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/organizations/${selectedOrg.id}/users`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
      ...
      if (data.success) {
        addNotification('User added to organization', 'success');
        // Refresh user lists
        handleManageUsers(selectedOrg);
      } else {
        addNotification(data.message || 'Failed to add user', 'error');
      }
    } catch (error) {
  ...
  const handleRemoveUser = async (userId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/organizations/${selectedOrg.id}/users/${userId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await response.json();
      ...
      if (data.success) {
        ...
        addNotification(data.message || 'Failed to remove user', 'error');
      }
    } catch (error) {
      console.error('Error removing user:', error);
      addNotification('Error removing user', 'error');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 text-sm">Create and manage organizations, assign users</p>
        </div>
        <button
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                      <span>Created by {org.creator_name}</span>
        ...
        <div className="modal-overlay" onClick={() => setShowCreateModal(false)}>
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p className="text-gray-600 mb-4">Use the "Create Organization" button in the People page to create new organizations with user assignments.</p>
              <button
                onClick={() => {
      ...
      {showEditModal && selectedOrg && (
        <div className="modal-overlay" onClick={() => setShowEditModal(false)}>
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdateOrganization} className="modal-body">
      ...
      {showUserModal && selectedOrg && (
        <div className="modal-overlay" onClick={() => setShowUserModal(false)}>
          <div className="modal-container" onClick={e => e.stopPropagation()} style={{maxWidth: '800px'}}>
            <div className="modal-header">
              <h2 className="modal-title">Manage Users - {selectedOrg.name}</h2>
            ...
            <div className="modal-body">
              <div className="grid md:grid-cols-2 gap-6">
                {/* Current Users */}
                <div>
                  <h4 className="font-semibold mb-3">Current Members ({orgUsers.length})</h4>
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {orgUsers.map(user => (
                      <div key={user.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          <div className="user-avatar-small">
                            <span className="avatar-icon">{user.role === 'admin' ? '👑' : '👤'}</span>
                          </div>
                          <div>
                ...
                {/* Available Users */}
                <div>
                  <h4 className="font-semibold mb-3">Available Users ({availableUsers.length})</h4>
                  <div className="space-y-2 max-h-60 overflow-y-auto">
